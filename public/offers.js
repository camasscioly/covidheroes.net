...
window.onload = () => {
  ...
  if (localStorage.getItem('name')) {
    ...
    function addEntry(title, author, date, tags, id, dom) {
      ...
      const fulfill = ` <button class="btn btn-danger" onclick="if (localStorage.getItem('name') !== '${author}') { window.location = '${window.location.origin}/requests/open?id=${id}' }">Open</button>`;
      document.querySelector(dom).innerHTML += `<tr id="${id}">
        ...
        <td><p>${tags}</p></td>
        <td>${id}</td>
        <td>${localStorage.getItem('name') === author ? fulfill + close : fulfill}</td>
      </tr>`;
    }
